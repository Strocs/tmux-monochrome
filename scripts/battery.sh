...
main() {
  bat_label=$(get_tmux_option "@strocs-battery-label" "♥")
  if [ "$bat_label" == false ]; then
    bat_label=""
  fi

  no_bat_label=$(get_tmux_option "@strocs-no-battery-label" "AC")
  if [ "$no_bat_label" == false ]; then
    no_bat_label=""
  fi

  show_bat_label=$(get_tmux_option "@strocs-show-battery-status" false)
