...
get_platform() {
  ...
  Linux)
    ...
    gpu_label=$(get_tmux_option "@strocs-force-gpu" false)
...
get_gpu() {
  gpu=$(get_platform)
  gpu_vram_percent=$(get_tmux_option "@strocs-gpu-vram-percent" false)
  if [[ "$gpu" == NVIDIA ]]; then
    if $gpu_vram_percent; then
      ...
      used_accuracy=$(get_tmux_option "@strocs-gpu-vram-used-accuracy" "d")
      total_accuracy=$(get_tmux_option "@strocs-gpu-vram-total-accuracy" "d")
...
main() {
  ...
  gpu_label=$(get_tmux_option "@strocs-gpu-vram-label" "VRAM")
  gpu_usage=$(get_gpu)
